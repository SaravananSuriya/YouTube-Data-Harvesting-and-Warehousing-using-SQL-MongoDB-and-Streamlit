...
conn = mysql.connector.connect(host = 'localhost', password = 'Saravanan123', user = 'root', port = 3307, database = 'project')
cur = conn.cursor()
try:
...
# horizontal menu
img = Image.open('C:\\Users\\WELCOME\\OneDrive\\Desktop\\saravanan\\Visual_Studio_Code\\images\\youtube.png')
st.set_page_config(page_title = 'Youtube Project', page_icon = img, layout = 'wide')
...
if selected == 'Home':
    ...
    if len(id) != 0:
      ...
      if name in names:
        ...
        def channel(channel_id):
          api_key = 'AIzaSyALcSm3xC8HQk-FFyVc5Sj4fBz5jNyuVXY'
  ...
  if choice == '1. What are the names of all the videos and their corresponding channels?':
    ...
    st.dataframe(df)
    figure = px.bar(df,x='Channel_Name',y='Video_Comment_count',color='Channel_Name',title='Videos which contain highest number of comments',orientation='v',height=600)
    st.plotly_chart(figure,use_container_width=True)
